...
If any of that gets too gnarly (gradients and padding are common offenders), the safe fallback is two-pronged:

1. **Always emit the canonical wrapper.** Keep the `<!-- wp:blockshot/canvas {...} -->` comment, the `<div>` with its required class list, `data-blockshot-canvas="true"`, and the base inline `style` covering width / minHeight / display / flex-direction / justify-content / fallback colors. Skipping or stripping these has distinct failure modes: omitting `data-blockshot-canvas` makes frontend export silently no-op; omitting the base inline style renders the canvas at the wrong size/layout; omitting any of the saved-markup pieces that `save.js` would have produced (classes, base style) drifts the content out of sync with the JSON attributes and trips Gutenberg's block validator when the user next opens the post.
2. **For gradient and padding specifically, don't put them in the JSON unless the saved `<div>` mirrors them.** Gutenberg parses those JSON attributes and expects the saved markup to match what `save.js` would output; a JSON-only gradient or padding is exactly the validation-drift case above. Pick one of:
   - **Hand-author the gradient/padding fully** in both the JSON comment **and** the saved `<div>` (the patterns above). Best for "create-and-forget" flows where no human will open the post.
   - **Omit those advanced JSON attributes entirely** and tell the user to set them in the editor and save once. Best when the values are easier to dial in visually.
